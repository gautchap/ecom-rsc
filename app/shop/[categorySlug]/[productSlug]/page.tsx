...
import { EnvelopeClosedIcon, ReloadIcon, SymbolIcon } from "@radix-ui/react-icons";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const productFiltered = category.Product.find(
        (product: Product) => product.name === decodeURI(params.productSlug) && product.categoryId === category.id
    );

    if (!productFiltered) notFound();

    return {
        title: productFiltered.name.charAt(0).toUpperCase() + productFiltered.name.slice(1),
    };
}
...
export default async function Page({ params }: Props) {
    ...
    const categoriesFiltered = categories.map((cat) => {
        cat.Product = cat.Product.filter((product) => product.name !== "Tshirt Large");
        return cat;
    });
        ...
        <section>
            <div className="flex justify-center gap-8 flex-wrap">
                ...
                <div className="max-w-xs">
                    ...
                    <p className="text-muted-foreground my-4">{productFiltered.description}</p>
                    ...
                    <Accordion type="single" collapsible className="w-full my-4">
                        <AccordionItem value="item-1">
                            ...
                            <AccordionContent>
                                Votre colis arrivera dans un délai de 3 à 5 jours ouvrables à votre lieu
                                d&apos;enlèvement ou dans le confort de votre domicile. ou dans le confort de votre
                                domicile.
                            </AccordionContent>
                        </AccordionItem>
                        <AccordionItem value="item-2">
                            ...
                            <AccordionContent>
                                La coupe n&apos;est pas parfaite ? Pas de souci - nous échangerons votre produit pour un
                                nouveau produit.
                            </AccordionContent>
                        </AccordionItem>
                        <AccordionItem value="item-3">
                            ...
                            <AccordionContent>
                                Retournez-nous simplement votre produit et nous vous rembourserons. Aucune question
                                n&apos;est posée. Nous ferons tout notre possible pour que votre retour se fasse sans
                                problème.
                            </AccordionContent>
                        </AccordionItem>
                    </Accordion>
